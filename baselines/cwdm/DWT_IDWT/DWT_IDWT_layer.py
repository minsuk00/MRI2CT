...
class DWT_2D_tiny(Module):
    ...
    def get_matrix(self):
        ...
        :return: self.matrix_low = \mathcal{L}, self.matrix_high = \mathcal{H}
        """
        # Include input_depth in L1 so matrix_h/g rows cover the largest spatial axis.
        # Original cWDM only used (H, W) which works for BraTS (D <= max(H, W)) but
        # silently truncates matrix_*_2 when depth is the largest axis (SynthRAD thorax).
