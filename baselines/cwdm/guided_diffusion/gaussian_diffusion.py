...
class GaussianDiffusion:
    ...
    def p_sample_loop(
        ...
        """
        # Default `time` to this diffusion's timestep count so SpacedDiffusion (ddim50 etc.)
        # iterates only over its respaced steps instead of the un-respaced 1000 default.
        if time is None:
    ...
    def training_losses(self, model,  x_start, t, classifier=None, model_kwargs=None, noise=None, labels=None,
        ...
        if model_kwargs is None:
            ...
            elif contr == 'ct':  # SynthRAD MR -> CT: single conditioning modality
                target = x_start['ct']
                cond_1 = x_start['mri']
                LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH = dwt(cond_1)
                cond_dwt = th.cat([LLL / 3., LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1)
                # Skip cond_2/cond_3 cat path below.

            else:
                print("This contrast can't be synthesized.")

            if contr != 'ct':
                # Concatenate conditions along the channel dimension (BraTS: 3 modalities)
                LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH = dwt(cond_1)
                cond_dwt = th.cat([LLL / 3., LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1)
