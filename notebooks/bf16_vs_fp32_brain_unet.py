"""Same bf16-vs-fp32 brain comparison as bf16_vs_fp32_brain.py, but for the
unet baseline checkpoint. Predictor is the U-Net directly on MRI."""
import os
import sys
...
SUBJ = "1BB005"
OUT = "/home/minsukc/MRI2CT/notebooks/bf16_vs_fp32_brain_unet.png"
device = torch.device("cuda")


@torch.inference_mode()
def infer(model, mri, val_ps, overlap, bsz, use_bf16):
    if use_bf16:
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            return sliding_window_inference(
                inputs=mri, roi_size=(val_ps,) * 3, sw_batch_size=bsz,
                predictor=model, overlap=overlap, device=device).float()
    return sliding_window_inference(
        inputs=mri, roi_size=(val_ps,) * 3, sw_batch_size=bsz,
        predictor=model, overlap=overlap, device=device).float()
...
def main():
    ...
    val_xform = get_cached_transforms(
        patch_size=cfg.get("patch_size", 128), res_mult=cfg.get("res_mult", 32),
        enforce_ras=True, mri_norm=cfg.get("mri_norm", "minmax"),
    ...
    pred_bf16 = unpad(infer(model, mri, val_ps, overlap, bsz, True), orig_shape)
    pred_fp32 = unpad(infer(model, mri, val_ps, overlap, bsz, False), orig_shape)
    ...
    fig.suptitle(f"UNet brain {SUBJ}  axial z={z}  |  window [-100,100] HU", y=1.0, fontsize=15)
