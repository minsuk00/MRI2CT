...
if [ -z "$SLURM_JOB_ID" ]; then
    ...
    JOB_NAME="${PREFIX}_${SPLIT_NAME}_dice-${DICE_W}_aug-${AUGMENT}_ep-${EPOCHS}"
    if [ ! -z "$RESUME_ID" ]; then
...
CMD="python $SCRIPT --split_file $SPLIT_FILE --dice_w $DICE_W --augment $AUGMENT --epochs $EPOCHS --steps_per_epoch $STEPS_PER_EPOCH --num_workers $NUM_WORKERS"
if [ ! -z "$RESUME_ID" ]; then
