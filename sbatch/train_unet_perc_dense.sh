...
# --- Configuration Area ---
# A/B equivalence test for the new local-squared-NCC (LNCC) perceptual loss, DENSE arm.
# Replaces the old (buggy) global NCC run y5tqp2bt. This arm uses the dense box-sum
# (single k^3 grouped conv) + torch.compile. Its twin (train_unet_perc_sep.sh) uses the
# separable box-sum (3x 1-D convs) + compile. If the separable factorization is correct,
# the two runs should train essentially identically (dense is slower but same math).
...
SEPARABLE="False"        # dense LNCC box-sum (slower, same math). compile is on via compile_mode=model
...
TAGS="bs4,perceptual,dice,lncc,dense" # Comma-separated extra WandB tags
