...
os.environ["WANDB_IGNORE_GLOBS"] = "*.pt;*.pth" # don't sync model weight files to wandb
...
def get_infinite_loader(loader):
    """
    Wraps a DataLoader in an infinite loop. 
    Prevents the overhead of restarting torchio.Queue workers.
    """
    while True:
        for batch in loader:
            yield batch
...
class DataPreprocessing(tio.Transform):
    ...
    def apply_transform(self, subject):
        ...
        # 2. Smart Padding (Multiple of 16/32 AND >= Patch Size)
        ...
        # 3. Smart Padding (Multiples of res_mult AND at least patch_size)
        current_padded_shape = subject['ct'].spatial_shape
        target_shape = []
        for dim in current_padded_shape:
            mult_16 = (int(dim) + self.res_mult-1) // self.res_mult * self.res_mult
            target_shape.append(max(self.patch_size, mult_16))
            
        padding_params = []
        for curr, targ in zip(current_padded_shape, target_shape):
            diff = int(targ - curr)
            padding_params.extend([0, diff]) # Append (0, diff). Pad at END
...
def get_reference_subject(subj_id, args):
    """
    Training Monitor용. 확인용 샘플 하나 가져옴
    """
    ...
    # 전처리 적용 후 CPU Tensor로 반환
    preprocess = DataPreprocessing(
        patch_size=args.patch_size, enable_safety_padding=False, res_mult=args.res_mult
    )
    subj = preprocess(subj)
    return {
        'id': subj_id,
        'mri': subj['mri'][tio.DATA].unsqueeze(0), # (1, 1, W, H, D)
        'ct': subj['ct'][tio.DATA].unsqueeze(0),
        'orig_shape': subj['original_shape'].tolist()
    }
    
def get_val_loader(data_path_list, args):
    """
    Creates a DataLoader for validation that yields FULL volumes
    """
    print(f"[DEBUG] ⚡ Loading {len(data_path_list)} validation subjects...")
    ...
    for item in data_path_list:
        subject = tio.Subject(
            ...
            subj_id=item['subj_id'] # Store ID for logging
        )
        # subj.load() # load to RAM
    ...
    ds = tio.SubjectsDataset(subjects, transform=preprocess)
    return torch.utils.data.DataLoader(ds, batch_size=1, shuffle=False, num_workers=0)

def get_train_loader(data_path_list, args):
    ...
    preprocess = DataPreprocessing(patch_size=args.patch_size, enable_safety_padding=use_safety, res_mult = args.res_mult)    
...
# ==========================================
@torch.no_grad()
def visualize_ct_feature_comparison(pred_ct, gt_ct, gt_mri, model, subj_id, root_dir, original_shape, epoch=None, use_wandb=False, idx=1, enable_pca=False):
    ...
    # 4. Dynamic Grid Creation
    num_cols = len(items) # Will be 5 (No PCA) or 8 (With PCA)
...
def log_aug_viz(ref_data, args, epoch):
    """
    ...
    try:
        # Tensor -> Subject 복원
        subj = tio.Subject(
            mri=tio.ScalarImage(tensor=ref_data['mri'][0].cpu()),
            ct=tio.ScalarImage(tensor=ref_data['ct'][0].cpu())
        )
...
# ==========================================
def train_one_epoch(model, loader_iter, optimizer, loss_fn, scaler, device, model_type, feat_extractor, args):
    model.train()
    ...
    progress = tqdm(range(args.steps_per_epoch), desc="Train Step", leave=False, dynamic_ncols=True)
...
@torch.no_grad()
def evaluate(model, feats_mri, ct_tensor, device, model_type, original_shape):
    model.eval()

    pred_ct_tensor = model(feats_mri)

    # Unpad & Metrics
    pred_unpad = unpad(pred_ct_tensor, original_shape)
    ct_unpad = unpad(ct_tensor, original_shape)
    
    metrics = compute_metrics(pred_unpad, ct_unpad)
    pred_ct_numpy = pred_ct_tensor.squeeze().cpu().numpy() # Return for viz
    
    return metrics, pred_ct_numpy
    
# Validation Helper
def run_validation(epoch, model, val_loader, device, feat_extractor, args, loss_fn, avg_train_loss = 0.0, viz_limit=3, train_monitor_data=None):
    model.eval()
    val_metrics = defaultdict(list)

    val_iter = tqdm(val_loader, desc=f"Val (Ep {epoch})", leave=False, dynamic_ncols=True)

    # 1. Standard Validation Loop
    for i, batch in enumerate(val_iter):
        mri = batch['mri'][tio.DATA].to(device)
        ct = batch['ct'][tio.DATA].to(device)
        orig_shape = batch['original_shape'][0].tolist() # Tensor -> List
        subj_id = batch['subj_id'][0]
        
        with torch.no_grad():
            feats = feat_extractor(mri)
            metrics, pred_np = evaluate(model, feats, ct, device, args.model_type, orig_shape)
            
            # Loss 계산 (옵션)
            pred_t = torch.from_numpy(pred_np).float().to(device).unsqueeze(0).unsqueeze(0)
            loss_val, _ = loss_fn(pred_t, ct, feat_extractor=feat_extractor)
            metrics['loss'] = loss_val.item()

        for k, v in metrics.items(): val_metrics[k].append(v)
        
        # Viz
        if args.wandb and i < viz_limit:
            tqdm.write(f"   🔎 Val Viz [{i+1}/{viz_limit}] ({subj_id})")
            visualize_ct_feature_comparison(pred_np, ct.squeeze().cpu().numpy(), mri.squeeze().cpu().numpy(), feat_extractor, subj_id, args.root_dir, orig_shape, epoch, True, i)

    # 2. Training Monitor Viz (학습 데이터 1개)
    if args.wandb and train_monitor_data:
        mri = train_monitor_data['mri'].to(device)
        ct = train_monitor_data['ct'].to(device)
        with torch.no_grad():
            feats = feat_extractor(mri)
            _, pred_np = evaluate(model, feats, ct, device, args.model_type, train_monitor_data['orig_shape'])
            
        visualize_ct_feature_comparison(pred_np, ct.squeeze().cpu().numpy(), mri.squeeze().cpu().numpy(), feat_extractor, f"{train_monitor_data['id']}_TRAIN", args.root_dir, train_monitor_data['orig_shape'], epoch, True, -1)
        
        # Aug Viz
        if args.augment:
            log_aug_viz(train_monitor_data, args, epoch)
    
    avg_metrics = {k: np.mean(v) for k, v in val_metrics.items()}
    
    log_str = f"Ep {epoch} | Train Loss: {avg_train_loss:.5f}"
...
def run_experiment(exp_config_dict):
    ...
    print(f"[DEBUG] 🏗️ Building and Compiling Anatomix {args.anatomix_weights}...")
    ...
    # 7. Create Loader
    def make_paths(subjs):
        return [{**get_subject_paths(args.root_dir, s), 'subj_id': s} for s in subjs]

    # 1. Loaders
    train_loader = get_train_loader(make_paths(train_subjects), args) # Queue (Train)
    train_iter = get_infinite_loader(train_loader)
    val_loader = get_val_loader(make_paths(val_subjects), args)     # DataLoader (Val)

    # 2. Monitor Data Capture (1명만)
    train_monitor_data = None
    if train_subjects:
        train_monitor_data = get_reference_subject(train_subjects[0], args)
    ...
    if args.sanity_check and not args.resume_wandb_id:
        run_validation(0, model, val_loader, device, feat_extractor, args, loss_fn, viz_limit=args.viz_limit, train_monitor_data=train_monitor_data)
    
    epoch_iter = tqdm(range(start_epoch, epochs + 1), desc="Epochs", leave=True, dynamic_ncols=True)
    for epoch in epoch_iter:
        epoch_start = time.time()
        
        loss, loss_comps, grad_norm = train_one_epoch(model, train_iter, optimizer, loss_fn, scaler, device, args.model_type, feat_extractor, args)
        ...
        if (epoch % args.val_interval == 0) or (epoch == epochs):
            run_validation(epoch, model, val_loader, device, feat_extractor, args, loss_fn, avg_train_loss=loss, viz_limit=args.viz_limit, train_monitor_data=train_monitor_data)
