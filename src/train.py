...
class Trainer:
    ...
    def _setup_data(self):
        ...
        self.train_loader = tio.SubjectsLoader(queue, batch_size=self.cfg.cnn_batch_size, num_workers=0)
    ...
    def train_epoch(self, epoch):
        ...
        for _ in pbar:
            ...
            for _ in range(self.cfg.accum_steps):
                ...
                loss, comps = self.loss_fn(pred, ct, feat_extractor=self.feat_extractor)
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                list(self.model.parameters()) + list(self.feat_extractor.parameters()) if self.cfg.finetune_feat_extractor else self.model.parameters(), 
                max_norm=1.0
            )
            self.optimizer.step()
    ...
    def train(self):
        print(f"[DEBUG] 🏁 Starting Loop: Ep {self.start_epoch} -> {self.cfg.epochs_cnn}")
        ...
        if self.cfg.sanity_check and not self.cfg.resume_wandb_id:
            ...
            tqdm.write(
                    f"Ep {epoch} | Train: {0:.4f} | Val: {avg_met.get('loss',0):.4f} | "
                    f"SSIM: {avg_met.get('ssim',0):.4f} | PSNR: {avg_met.get('psnr',0):.2f}"
                )

        global_pbar = tqdm(
            range(self.start_epoch, self.cfg.epochs_cnn + 1),
            desc="🚀 Total Progress",
            initial=self.start_epoch,
            total=self.cfg.epochs_cnn,
            dynamic_ncols=True,
            unit="ep"
        )
        ...
        for epoch in global_pbar:
            ...
            if epoch % self.cfg.val_interval == 0 or epoch == self.cfg.epochs_cnn:
        ...
        self.save_checkpoint(self.cfg.epochs_cnn, is_final=True)
