...
from src.utils import count_parameters, get_ram_info
...
SEG_CONFIG = {
    ...
    "finetune": True,
    "compile_mode": "full",  # Options: None, "model", "full"
    "few_shot": False,
    "few_shot_amount": 1,
...
class SegTrainer:
    ...
    def _setup_model(self):
        ...
        # Optional Step-Level Compilation
        if self.cfg.get("compile_mode") == "full":
            tqdm.write("[SegTrainer] 🚀 Compiling Training Step (mode=full)...")
    ...
    def train(self):
        ...
        for epoch in epoch_pbar:
            ...
            for batch_data in train_pbar:
                ...
                inputs = batch_data["image"].to(self.device).as_tensor()
                labels = batch_data["label"].to(self.device).as_tensor()

                # Call the compiled (or not) step
                outputs, loss, grad_norm = self.train_step(inputs, labels)
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for i, val_data in enumerate(tqdm(self.val_loader, desc="Validating", leave=False)):
                # Keep MetaTensors for affine access
                val_images_meta = val_data["image"].to(self.device)
                val_labels_meta = val_data["label"].to(self.device)
                subj_id = val_data["subj_id"][0]

                affine = val_images_meta.affine[0].cpu().numpy()

                # Convert to plain tensors for model
                val_images = val_images_meta.as_tensor()
                val_labels = val_labels_meta.as_tensor()
                ...
                if self.cfg["save_val_volumes"] and i in self.val_viz_indices:
                    ...
                    # Use the affine captured earlier from MetaTensor
